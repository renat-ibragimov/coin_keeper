...
На 2026-09-03: этапы 0–3 завершены — бэкенд с аутентификацией, перенесённой базой
коллекции, API каталога, коллекции, расходов, серий и дашборда задеплоен на
`coins.renat-ibragimov.com` (Swagger — `/api/v1/docs`). Этап 4 (фронтенд) в работе:
готовы вход и регистрация, каталог, дашборд «Огляд» и карточка монеты с историей цен;
следующая часть — коллекция и добавление покупки, личные позиции, серии, «Не вистачає»,
настройки, выкладка фронтенда из CI. Затем — демо владельцу коллекции
(`docs/11-roadmap.md`).
