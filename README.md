...
Этап 0 (документация) завершён. Следующий — каркас бэкенда и рабочий контур выкладки:
схема, аутентификация с подтверждением email и восстановлением пароля, отправка почты,
ограничение частоты, а также сервер, домен и CI/CD, разворачивающий `main` на домен
