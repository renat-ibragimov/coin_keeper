...
первая же личная запись на проде («В дорозі») осталась непереведённой — ключ Anthropic
рабочий, `translate_short_phrase` при ручном вызове отработал верно, но живой запрос
почему-то не долетел до модели, и ни одной строки в логах API не было — молчаливый путь
`_find_or_create`/`translate_in_background` не оставлял следов ни на одном раннем
`return`. Добавлено логирование на каждый такой выход (нет ключа, запись пропала до
перевода, Haiku не вернул ожидаемый tool-use, исключение при вызове API) — воспроизвести
раз не удалось, но при повторении это будет видно в `docker logs`, а не только строкой
`name_en_source = manual` в БД.
