...
**Сопутствующие расходы той же покупкой** (2026-09-14): в транзакцию может входить ещё
несколько расходов — доставка, холдер, грейдинг, — с суммой и валютой каждый, а датой и
продавцом самой покупки. Курсы **всех** валют запроса разрешаются до первой вставки, то есть
доставка в валюте без курса отклоняет покупку целиком.

Связь у них слабее, чем у расхода `coin_purchase`: только `catalog_item_id`,
без `collection_item_id`. Отсюда обе стороны:

- удаление такого расхода из журнала «Гроші» — это просто удаление расхода, монета и
  экземпляр остаются;
- удаление экземпляра (п. 10) забирает с собой только свой `coin_purchase`, а доставка
  остаётся в журнале: деньги были потрачены независимо от того, в коллекции ли ещё монета.

Иначе доставку пришлось бы либо молча удалять вместе с монетой, либо считать покупкой — и в
журнале на неё смотрели бы иконки, которые удаляют экземпляр.
