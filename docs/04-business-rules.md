...
**В форме «Додати» (2026-09-14) три поля ведут себя по-разному, и это осознанно.**
Матеріал — словарь **плюс** свободный ввод в одном поле (`Combobox`): он обязателен, а
словарь засеян тем, что есть в подтверждённом каталоге, и про большинство эмитентов молчит.
Гурт и якість — только словарные выпадашки: они необязательны, а вписанный от руки гурт
никогда не совпал бы с тем, что пишет парсер, и превратился бы в ту самую замусоренность,
которую чинили миграции `0007` и `0010`. Полные словари для формы отдают
`GET /materials`, `GET /edge-types`, `GET /quality-types` (`03-api-contract.md`) — их не
надо путать с `GET /catalog/materials`, который сужает фильтр до реально встречающегося.
