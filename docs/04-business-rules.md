...
(миграция `0012`), по умолчанию `false`. `GET /catalog` (`CatalogRepository._filter_conditions`)
скрывает любую запись с непустым `packaging_of_id`, пока владелец её не включил в
настройках (`PATCH /bootstrap/settings`); карточка по прямой ссылке (`GET /catalog/{id}`)
