...
- уже имеющаяся ссылка на ua-coins (`price_source_links`, `source_key` или URL последнего
  общего снимка цены) — ссылка не догадка, и таких 594;
...
название, серия, номинал структурой, год, дата ввода, тираж, материал, масса, диаметр.
Только монеты — набор это продукт, а обиходные в нумизматическом каталоге не числятся.
Идемпотентность по `source_key = 'nbu:<id>'`, который схема и так держит уникальным
среди общих записей.
