...
внутри того же `classify()`. Миграция legacy-данных (`app/legacy_migration/media.py`) —
единственный сегодняшний путь, который явно передаёт `remove_background=False`: это разовая
транскрипция файлов из старой базы как есть, а не решение резать им фон.

### Runbook

Не запускать два прогона скрипта одновременно (в том числе `--trim` рядом с обычным) — оба
читают и пишут одни и те же строки `media_files` по одному и тому же `storage_key`.
