...
**Правило.** Режем только то, в чём уверены: белый фон **и** круглый объект.

- Все четыре угла фото — «почти белые» (консервативный порог по каждому каналу и по разбросу
  между каналами, чтобы кремовый или голубоватый фон не прошёл за белый). Иначе —
  `skip:not_white_bg`.
...
- Фон должен «обнимать» объект: если объект достаёт до края кадра — `skip:object_touches_border`
  (целая монета так не сфотографирована).
...
Тёмные и цветные фоны, а также прямоугольные объекты не трогаем никогда — это осознанное
ограничение, а не недоработка классификатора.
...
общая для украинского конвейера (`app/ukraine_pipeline/photos.py`,
`app/ukraine_pipeline/circ_photos.py`) и будущей ручной загрузки этапа 6. Флаг
`remove_background: bool = True`: белое круглое фото режется сразу при сохранении, без отдельного
прохода постфактум. Миграция legacy-данных (`app/legacy_migration/media.py`) — единственный
сегодняшний путь, который явно передаёт `remove_background=False`: это разовая транскрипция
файлов из старой базы как есть, а не решение резать им фон.
...
# migration-reports/nobg-review.html (простыня до/после для cut, список skip по причинам)

# применить только к вердиктам cut
...
срезает префикс через `handle_path` (`10-infra.md`, раздел «Caddy»), а `Host` не
