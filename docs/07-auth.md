...
## Google OAuth и линковка аккаунтов (решение владельца 2026-09-15)

Новое в MVP — раньше OAuth стоял в «Что откладываем» ниже («по желанию»), теперь это
часть MVP. Детали проектирования не расписаны — отдельная задача.

Согласовано:
- Вход через Google — наравне со входом по email и паролю.
- **Линковка аккаунтов**: пользователь, зарегистрированный по email, может привязать
  Google-вход на тот же адрес; пользователь, вошедший через Google, может задать пароль
  и получить обычный вход по email — в любом порядке.

Не спроектировано, отдельная задача:
- Флоу линковки в интерфейсе — откуда инициируется, что видит пользователь.
- Поведение при конфликте: Google-аккаунт с email, который уже зарегистрирован по паролю,
  но не подтверждён или принадлежит другому человеку.
- Модель хранения — отдельная таблица идентичностей провайдера или поля на `users`; какие
  данные Google отдаёт и что из них сохраняем.
- Эндпоинты и их место в `03-api-contract.md`.

<!-- TODO(owner): детали флоу линковки Google-аккаунта — отдельная задача, решение 2026-09-15 -->
