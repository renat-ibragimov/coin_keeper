...
- **Единый паттерн пустых состояний** (решение владельца 2026-09-07, ужесточено тем же
  ...
  кнопки дій — включно з «+ Додати покупку» і «+ Додати витрату»)
  ...
  і secondary «Додати покупку» (`card.addPurchase`, → `/collection/add`). Тексти
  заголовка й опису — свої для кожної сторінки, кнопки — ні.
- **Плашка як картка** (уточнено 2026-09-07, двічі — перша версія розтягувала картку на
  ...
  тому підрізаний свідомо (наприклад, `series.emptyCollectionText`, 2026-09-07).
...
- **Монети** (`/collection/coins`): замість порожньої колекції — «У вашій колекції ще
  ...
  колекції".» на картці та кнопки «Перейти до каталогу» / «Додати покупку» (та сама пара,
  що на решті трьох сторінок — «Загальні рішення»). Шапка без жодної дії, KPI-плитки й панель фільтрів у цьому стані
...
- **Серії** (`/collection/series`): список серій із суцільними 0 % **не показуємо** —
  ...
  влізало в картку однакового розміру) + «Перейти до каталогу» / «Додати покупку».
  Стан «для цієї країни серій немає»
...
- **Гроші** (`/collection/money`): «Фінансової історії поки немає. Коли ви додасте
  ...
  до каталогу» / «Додати покупку» — та сама пара,
...
- **Порожній стан** (єдиний паттерн, `08-ui-map.md`, «Загальні рішення»): при порожній
  ...
  покупку». Стан «покупок немає, супутні
