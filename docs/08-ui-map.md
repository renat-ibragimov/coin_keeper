...
- **Owned-подсветка в каталоге** (решение владельца 2026-09-07, по просьбе брата — чтобы монеты
  ...
  таблицы для монеты `owned` получают лёгкую зелёную тонировку поверхности (новый токен
  `--color-owned-surface`, ~5 % `--color-owned` поверх `--color-surface-raised`, свой в каждой
  теме) плюс рамку/иконку в тон `--color-owned` (не `--color-owned-icon` — этот токен неизменен
...
- **Годовые фильтры «Рік від/до» — выпадашки, а не `<input type=number>`** (решение владельца
  2026-09-07: долистать спиннером от 1 до 1995 — квест). Общий `Select` (`shared/ui/Select.tsx`)
  теперь умеет `<optgroup>`: опции — годы по убыванию (свежие сверху), сгруппированы по
  десятилетиям («2020-ті», «2010-ті», …, рендерится как `text-transform: uppercase`, тот же
  приём, что у заголовков таблицы каталога); первая опция — пустая «—». Границы считает
  `shared/lib/yearRange.ts`: выбрана страна → её `minYear/maxYear` из блока 1 (`CountryOut`);
  страна не выбрана или её собственных границ нет → min/max по всем странам загруженного
  справочника; справочник пуст — фолбэк 1900..текущий год. Каталожная панель берёт каталожные
  границы (общий `GET /countries`), коллекционная — свои (`GET /collection/countries`). «До» не
  предлагает год раньше выбранного «від» и наоборот — оба поля независимо сужают одни и те же
  общие границы значением соседнего поля. Смена страны пересчитывает границы и **клампит**
  выбранные `yearFrom`/`yearTo` к ближайшей границе нового диапазона, а не сбрасывает их в пусто.
...
- «Мої серії» вместо «Улюблених серій» из мока (и вместо прежнего
  ...
  Контейнер списка — фиксированной высоты (~6 строк, 300px); коротший список
  просто не дотягує до неї (без скролбару), довший — прокручується, і остання
  видима строка навмисно обрізана краєм контейнера як візуальне запрошення до
  прокрутки. Пустое состояние («Ще не почато жодної серії.») — только когда
  начатых серий нет вовсе; ссылка «Усі серії» остаётся в заголовке блока.
