...
- **Навигация** (этап 4) — двухконтекстная модель вместо плоских семи разделов. В шапке
  ...
  без отдельного лэйаута на экран). Справа вместо имени и кнопки выхода — меню аккаунта
...
- Везде, где показывается настоящее фото (плитки списков, миниатюры таблиц, карточка
  ...
  ничего не обрезается по краям зоны 16:9. Плашка (`--color-photo-plaque`) и
...
- Четыре плитки, **каждая — ссылка целиком** (растянутый `Link`, а не `div` с
  `onClick`: клавиатура и середня кнопка миші працюють): «Монет у колекції»
  (`collectionItems`) → `/collection/coins`; «Не вистачає» (`missingItems`) →
  `/collection/missing`; «Комплектність» (%) → `/collection/series`; «Поточна
  оцінка» (`marketValueUah`) → `/collection/money` — заменила плитку «Країн у
  каталозі» в главном ряду KPI. Блок «Розподіл за країнами» ниже по странице
  никуда не делся — країни остались, просто не в KPI.
- «Найближчі до завершення» вместо «Улюблених серій» из мока: серии из `seriesBreakdown`
  (в API это топ-6 по размеру, с `id` серии), незавершённые, по убыванию доли, с
  прогресс-кольцом «X з Y». Строка серии целиком кликабельна (тот же паттерн
  растянутой ссылки, что в картках каталога) → `/collection/series/{id}`. Миниатюры
  недостающих монет — часть 3.
- **Контекстный CTA** под списком: для самой близкой к завершению незавершённої серії —
  «До завершення серії «{назва}» залишилося {N} монет» (плюрализація через i18n) і
  кнопка «Переглянути відсутні» → `/collection/missing?seriesId={id}` (Missing бере
  фільтр `seriesId` з URL сам, предфільтр спрацьовує без додаткового коду). Немає
  незавершених серій або колекція порожня — блоку немає.
