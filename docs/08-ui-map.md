...
Тёмная тема legacy (`#08090b`, золотой акцент) выглядела хорошо, но остаётся **после MVP**
как вторая тема. Делать сразу две — удвоить работу над каждым экраном ради вкусовщины
на старте.
...
- Не тащим тёмную тему параллельно светлой: сначала одна, полностью.
...
Тема в legacy была тёмная: фон `#08090b`, акцент — тёплое золото. **В MVP основная тема
светлая**, тёмная переезжает на после MVP — см. «Визуальное направление» выше.
...
| Структуру карточки монеты | **Тему меняем**: legacy был тёмный, MVP — светлый «бумажный»; тёмная после MVP |
...
- Светлая «бумажная» тема — единственная в MVP. Цвета и шрифты задаются токенами, чтобы
  вторая тема после MVP не потребовала переписывать компоненты.
