...
| Створити свою позицію | `/catalog/new` | заглушка → часть 3 |
| Імпорт з uCoin | `/import` | заглушка → этап 6 |
| Колекція | `/collection` | заглушка → часть 3 |
| Додати покупку | `/collection/new?catalogItemId=` | заглушка → часть 3 |
| Серії | `/series` | заглушка → часть 3 |
| Не вистачає | `/missing` | заглушка → часть 3 |
| Налаштування | `/settings` | заглушка → часть 3 |

Заглушка — экран `ComingSoon` с названием раздела и ссылкой в каталог; роут существует,
чтобы ссылки из готовых экранов уже вели куда надо.

### Общие решения, принятые в коде

- **Навигация** — шесть разделов в шапке: Огляд, Каталог, Серії, Колекція, Не вистачає,
  Налаштування. На телефоне — нижняя панель из трёх (Огляд, Каталог, Колекція) и кнопка
  «Ще» с остальными и выходом. После входа пользователь попадает на Огляд.
