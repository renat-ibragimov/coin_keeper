...
`/collection/coins/:id/edit`; неизвестный путь — на `/collection`.
...
- Заголовок «Огляд колекції» и подзаголовок центрированы через `PageHeader
  ...
  Монети, Каталог, Серії, Не вистачає, Гроші и картка серії тоже на нём
  (заголовок/подзаголовок/дії стопкою по центру; `above`-слот с хлібними
  крихтами, где он есть, остаётся выключенным влево).
- Четыре плитки, **каждая — ссылка целиком** (растянутый `Link`, а не `div` с
  ...
  паддинг плиток задан в самом `StatTile`, поэтому Огляд, Монети и «Не
  вистачає» рендерят плитки одного размера без экранных переопределений.
