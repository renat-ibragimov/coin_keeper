...
| Комплектність (група за обраним полем — серія/рік/номінал/метал) та сторінка групи | `/collection/completeness`, `/collection/completeness/:groupBy/:value` | **реализовано** (часть 3, обобщено с «Серії» 2026-09-23) |
...
- Четыре плитки, **каждая — ссылка целиком** (растянутый `Link`, а не `div` с
  ...
  `/collection/completeness`; «Поточна оцінка» (`marketValueUah`) → `/collection/money`
...
- **Комплектність** (`/collection/completeness`, до 2026-09-23 — «Серії»): список груп
  обраного поля (`groupBy` — серія/рік/номінал/метал, перемикач `Tabs` у тулбарі поруч зі
  скоупом і сортуванням, за замовчуванням «Серія»). Порожня колекція цілком (`isEmpty`) —
  картка EmptyState «Тут поки порожньо» + «Перейти до каталогу» / «Додати покупку», решта
