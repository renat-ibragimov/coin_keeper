...
- **Единый паттерн пустых состояний** (решение владельца 2026-09-07, ужесточено тем же
  ...
  немає.
- **Плашка як картка**: `EmptyState` (`shared/ui/States.tsx`) приймає `variant="card"` —
  той самий `surface-raised`/border/`radius-lg`, що й у звичайних карток (`Card`), і саме
  так рендериться на всіх чотирьох сторінках вище. Внутрішньоконтентні «нічого не
  знайдено» після фільтрів/пошуку (каталог, «Мої монети», серії) лишаються без рамки
  (`variant="plain"`, дефолт). `EmptyState`/`ErrorState` більше не мають текстовий гліф
  дефолтом — тільки іконки lucide, `strokeWidth={1.75}`, як у плиток. Одна сутність —
  одна іконка:

  | Страница / состояние                                              | Иконка         |
  |---|---|
  | Огляд колекції, Мої монети                                        | `Coins`        |
  | Серії                                                             | `Layers`       |
  | Гроші                                                             | `Wallet`       |
