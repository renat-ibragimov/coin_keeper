...
После открытия каталога для гостей `/` ведёт в `/catalog` без сессии и в `/collection`
после входа. Гость может открыть `/catalog/:id` и четыре корневых раздела коллекции;
последние показывают пустые состояния с кнопкой входа. Вход и регистрация с гостевых
страниц открываются в модальном окне, успешный вход оставляет на текущем маршруте.
Прямые `/login` и `/register` сохранены. Личные формы остаются закрытыми.

Редиректы со старых путей (этап 4): `/dashboard` → `/collection`; `/series[/:id]` →
