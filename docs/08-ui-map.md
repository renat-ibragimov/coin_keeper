...
4. **Саджеста нет, человек пишет своё** → под названием раскрывается блок **«Про монету»**.
   ...
   - **Матеріал**, **номінал** и **серія** — по одному `Combobox` на каждое: список
     ...
     для страны без справочника показывались неактивными полями с извинением, см.
     `docs/current_ref/img.png`). Совпало со строкой справочника — уходит id, не совпало —
     текст. Своя серия при этом только показывается: комплектность считается по общим
     сериям, о чём поле и пишет в подсказке (`04-business-rules.md`, п. 14).
   - **Гурт** и **якість** — только выпадашки словарей (`GET /edge-types`,
     ...
     пишет парсер.
   - **Рік випуску** — тот же `Combobox`, что «Рік від/до» в фильтрах каталога: список по
     годам этой страны плюс свободный ввод, от свежего года к старому.
