...
- **Навигация** (этап 4, шапка и мобильная нижняя панель переработаны 2026-09-09) —
  ...
  `/collection/add`) і «Імпортувати з uCoin» (ghost, `/import`).
...
- **Единый паттерн пустых состояний** (решение владельца 2026-09-07, ужесточено тем же
  ...
  і secondary «Додати покупку» (`card.addPurchase`, → `/collection/add`); під ними
...
- `isEmpty` → «Колекція поки порожня» / «Знайдіть першу монету в каталозі та
  ...
  єдина пара «Перейти до каталогу» / «Додати покупку» (`/collection/add`), як і на
...
4. **Саджеста нет, человек пишет своё** → под названием раскрывается блок **«Про монету»**.
   ...
   деталей» — тираж, вага, діаметр, товщина, гурт, якість карбування, форма, KM# / UC# /
   Numista, нотатка про монету. Обязательных полей пять: країна, назва, рік, тип, матеріал
   (решение владельца 2026-09-14); год обязателен, потому что колонка `NOT NULL` и на ней
   держится комплектность серий.
   - **Матеріал** — `Combobox`: список из `GET /materials` плюс свободный ввод. Совпало со
     справочником — уходит `compositionId`, не совпало — текст в `material`. **Гурт** и
     **якість** — только выпадашки словарей (`GET /edge-types`, `GET /quality-types`):
     вписанное от руки значение никогда не совпало бы с тем, что пишет парсер
     (`04-business-rules.md`, п. 14).
   - **Номінал** и **серія** — справочники по стране. Пустой справочник номиналов — поле
     показывается неактивным с подсказкой «для цієї країни номіналів у довіднику немає»:
     свободного номинала в модели нет. Серию пользователь выбирает, но не заводит — серии
     общие и их создаёт только администратор.
   - Отправляется это одним запросом `POST /collection` с вложенным `newCatalogItem`
     (`03-api-contract.md`), а не двумя подряд: иначе упавшая покупка оставляла бы позицию
     без экземпляра.
...
тот же выбор «країна + назва», но здесь он только **ссылается** на существующую позицию —
общую или свою личную — и ничего не создаёт. Выбранная монета уходит в `catalogItemId`,
очищается ссылкой «Обрати іншу монету». `seriesId` в интерфейсе не выводим.

### Точки входа

Все кнопки «+ Додати покупку» по приложению (колекція, Огляд, Серії, пустые состояния,
плитки и таблицы каталога, картка монети) ведут на `/collection/add`, с `?catalogItemId=`
там, где монета известна. «+ Додати витрату» на `/collection/money` — переход на
...
- **Таблиця**: колонка «Опис» для `coin_purchase` — назва монети (`coinTitle` з
  ...
  монети посиланням: прив'язку, яку людина зробила у формі, має бути видно (2026-09-14). Права колонка дій для рядків із покупок
