...
- **Единый паттерн пустых состояний** (решение владельца 2026-09-07, ужесточено тем же
  ...
  і secondary «Додати покупку» (`card.addPurchase`, → `/collection/coins/new`); тексти
  заголовка й опису — свої для кожної сторінки, кнопки — ні. Виняток: на «Мої монети» під
  парою кнопок — тиха текстова посилання «Імпортувати з uCoin» (`catalog.importUcoin`,
  → `/import`), бо кнопка в шапці на порожній сторінці схована, а іншого входу в імпорт
  немає.
- **Плашка як картка** (уточнено 2026-09-07 — попередня версія розтягувала картку на всю
  ширину сторінки): `EmptyState` (`shared/ui/States.tsx`) приймає `variant="card"` — той
  самий `surface-raised`/border/`radius-lg`, що й у звичайних карток (`Card`, але без її
  власного `padding` — своє вже несе `.state`, подвійний відступ картку тільки роздував),
  зафіксована ширина `max-width: 480px` і центрування (`margin: 0 auto`) — однакові на всіх
  чотирьох сторінках, а не «на всю ширину контейнера» як раніше. Відстань від підзаголовка
  до картки теж однакова скрізь (`space-5`, 24px): «Огляд» має тугіший `gap` контейнера
  сторінки (`space-3`, для щільного непорожнього тіла) — порожній стан там сидить в обгортці
  з власним `margin-top`, що компенсує різницю, а не в самому `.page`. Внутрішньоконтентні
  «нічого не знайдено» після фільтрів/пошуку (каталог, «Мої монети», серії) лишаються без
  рамки (`variant="plain"`, дефолт, на всю ширину). `EmptyState`/`ErrorState` більше не
  мають текстовий гліф дефолтом — тільки іконки lucide, `strokeWidth={1.75}`, як у плиток.
  Одна сутність — одна іконка (Огляд і Мої монети навмисно різні, щоб не дублювати
  іконку на сусідніх вкладках):
...
- **Монети** (`/collection/coins`): замість порожньої колекції — «У вашій колекції ще
  ...
  «Імпортувати з uCoin» під ними (єдиний виняток із пари кнопок — іншого входу в імпорт
  немає, коли кнопка шапки на порожній сторінці схована). Шапка без жодної дії,
  KPI-плитки й панель фільтрів у цьому стані зовсім не рендеряться (єдиний паттерн,
  «Загальні рішення»).
...
- **Гроші** (`/collection/money`): «Фінансової історії поки немає. Коли ви додасте
  ...
  до каталогу» / «Додати покупку» — та сама пара, що й скрізь. Шапка без «+ Додати
  витрату», KPI, графіки й чипси категорій у цьому стані не рендеряться (єдиний паттерн,
  «Загальні рішення»). Стан «покупок немає, супутні витрати додайте вручну» — окремий,
...
- **Порожній стан** (єдиний паттерн, `08-ui-map.md`, «Загальні рішення»): при порожній
  ...
  покупку». Стан «покупок немає, супутні витрати додайте вручну» — окремий, для
