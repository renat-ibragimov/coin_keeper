...
- **Навигация** (этап 4, шапка и мобильная нижняя панель переработаны 2026-09-09) —
  ...
  Серії, Гроші (реализована внутри `AppLayout` по `location`, без отдельного лэйаута на
  ...
  Каталог, Огляд, Монети, Серії, **Гроші** (весь простір «Ще» звільнило перенесення
...
(`countries.catalog_confirmed`, `04-business-rules.md`, п. 13a). Сегодня это одна Украина.
Позиция, серия или монета любой другой страны — хоть общая с затравки, хоть только что
заведённая пользователем личная, хоть уже купленная — в списке каталога, поиске и серіях не
появляется вовсе, ни при каком `scope`. Она никуда не девается: открывается по прямой
ссылке и видна в «Моя колекція» как обычно, просто не считается «каталогом».
