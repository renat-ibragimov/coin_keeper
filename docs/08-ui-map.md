...
- **Деньги и даты** — форматируются хелперами `shared/lib/format.ts` из строк API,
  цифры табличные.
- **Единый паттерн пустых состояний** (решение владельца 2026-09-07): у по-настоящему
  пустой страницы — только шапка (`PageHeader`) и `EmptyState`, без нулевой обвязки над
  ним. Для «Мої монети» и «Гроші» это значит: при `collectionEmpty` (пустой bootstrap,
  как на Огляді и Серіях) KPI-плитки, фільтри/графіки и чипсы категорий вообще не
  рендерятся — только заголовок, главное действие в шапке («+ Додати покупку» /
  «+ Додати витрату») и сам `EmptyState`. Скелетоны загрузки это не затрагивает — речь о
  подтверждённой пустоте из `bootstrap.dashboard.isEmpty`, а не о состоянии «данные ещё
  грузятся». `EmptyState`/`ErrorState` (`shared/ui/States.tsx`) больше не имеют текстовый
  глиф дефолтом — только иконки lucide, `strokeWidth={1.75}`, как у плиток. Одна сущность
  — одна иконка:
...
- `isEmpty` → «Колекція поки порожня» / «Знайдіть першу монету в каталозі та
  ...
  «Додати покупку» (`/collection/coins/new`).
...
- **Монети** (`/collection/coins`): замість порожньої колекції — «У вашій колекції ще
  ...
  колекції".» + кнопка «Перейти до каталогу». KPI-плитки й панель фільтрів у цьому стані
  зовсім не рендеряться (єдиний паттерн, «Загальні рішення»).
...
- **Гроші** (`/collection/money`): «Фінансової історії поки немає. Коли ви додасте
  ...
  (обидва — в каталог); KPI, графіки й чипси категорій у цьому стані не рендеряться
  (єдиний паттерн, «Загальні рішення»). Стан «покупок немає, супутні витрати додайте
  вручну» лишається для непорожньої колекції без жодної витрати.
...
- **Порожній стан** (єдиний паттерн, `08-ui-map.md`, «Загальні рішення»): при порожній
  колекції (`collectionEmpty`) KPI, графіки й чипси не рендеряться зовсім — тільки шапка
  й `EmptyState` (іконка `Wallet`): «Фінансової історії поки немає. Коли ви додасте
  покупку, тут з'являться витрати, поточна оцінка та різниця.» + «Додати першу монету»
  (в каталог). Стан «покупок немає, супутні витрати додайте вручну» — окремий, для
  непорожньої колекції без жодної витрати; KPI/графіки/чипси в ньому теж не показуються
  (нема даних для них), лишається шапка й `EmptyState`.
