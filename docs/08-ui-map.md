...
- **Каталог, фильтры и CTA** (промпт 2): на десктопе боковой панели больше нет — фильтры
  ...
  в `localStorage` браузера отдельно для каталога и коллекции (`ck.viewMode.catalog` /
  `ck.viewMode.collection`, общий хук `shared/lib/useStoredViewMode`, тот же приём, что у
  `ThemeProvider`): при заходе без явного `?view=` в адресе подставляется запомненное значение,
  переключение тумблера пишет и в адрес, и в `localStorage`; ссылка с явным `?view=` всегда
  побеждает запомненное. Те же два значения редактируются в «Налаштування» → «Вигляд за
  замовчуванням» (два селекти — «Каталог» / «Мої монети»), это удобство того же браузера, а не
  профиль пользователя (перенос на бэкенд — `BACKLOG.md`). На плитке и в таблице —
...
- **Годовые фильтры «Рік від/до» — редактируемое поле со стилизованной выпадашкой
  (`shared/ui/Combobox.tsx`)** (решение владельца 2026-09-07, три захода подряд: спиннер
  `<input type=number>` от 1 до 1995 — квest; выпадашка-`Select` с `<optgroup>` по десятилетиям —
  ручной ввод всё равно нужен, группировка мешает; нативный `<input list> + <datalist>` —
  подсказку рисует браузер своим системным стилем, чёрным прямоугольником поверх всего интерфейса,
  никак не тематизируется). `Combobox` — свой компонент: текстовое поле в стиле `Input` (можно
  вписать год руками) плюс выпадающий список под ним в стиле меню `Select` (та же карточка,
  тени, `.option`/`.optionActive`/`.check`) — открывается по фокусу/клику на шеврон, список не
  ограничивает ввод. Список — года по возрастанию (сначала минимум, потом максимум), без
  группировки по десятилетиям и без пустой первой опции (это текстовое поле, очистка — стереть
  текст). Границы считает `shared/lib/yearRange.ts`: выбрана страна → её `minYear/maxYear` из
  блока 1 (`CountryOut`); страна не выбрана или её собственных границ нет → min/max по всем
  странам загруженного справочника; справочник пуст — фолбэк 1900..текущий год. Каталожная
  панель берёт каталожные границы (общий `GET /countries`), коллекционная — свои
  (`GET /collection/countries`). «До» не подсказывает год раньше выбранного «від» и наоборот —
  оба поля независимо сужают одни и те же общие границы значением соседнего поля; список — только
  подсказка, а не ограничение, вписать год вне списка можно всегда. Смена страны пересчитывает
  границы и **клампит** выбранные `yearFrom`/`yearTo` к ближайшей границе нового диапазона, а не
  сбрасывает их в пусто.
...
- «Мої серії» вместо «Улюблених серій» из мока (и вместо прежнего
  ...
  каталога) → `/collection/series/{id}`. Миниатюры недостающих монет — часть 3.
  ...
  стилизованный скролбар (`scrollbar-width: thin`, цвет — `--color-border-strong`,
  + webkit-фолбэк). Пустое состояние («Ще не почато жодної серії.») — только
