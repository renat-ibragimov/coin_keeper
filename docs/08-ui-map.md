...
- **Таблица каталога не должна дышать** (замечания владельца 2026-09-09). Ширины колонок
  ...
  клиенте (`02-data-model.md`, `materials`).
...
- **Таблиця**: колонка «Опис» для `coin_purchase` — назва монети (`coinTitle` з
  ...
  порожня — дубль «з покупки монети» прибрано; для ручних записів там, як і раніше,
  «Редагувати»/«Видалити». Колонки «Вартість» і зарезервована гривнева — без змін.

  Уточнення 2026-09-09 (зауваження власника: «якийсь прочерк у кінці таблиці не ясно
  навіщо»): у порожній комірці дій тепер справді порожньо, а не прочерк із тултипом —
  прочерк читався як відсутнє значення. Обидві безіменні колонки отримали заголовки:
  «Дії» та «У доларах» (та сама зарезервована під курс НБУ колонка, `docs/BACKLOG.md`);
  поки в ній немає значень, вона показує прочерк приглушеним тоном і не сортується.
