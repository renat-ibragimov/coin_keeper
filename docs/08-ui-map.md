...
- **Комплектність** (`/collection/completeness`, до 2026-09-23 — «Серії»): список груп
  обраного поля (`groupBy` — серія/рік/номінал/матеріал/гурт/якість карбування, перемикач
  `Tabs` у тулбарі поруч зі скоупом і сортуванням, за замовчуванням «Серія»). Праворуч від
  перемикача — окрема випадашка «Цінність металу» (усі/дорогоцінні/недорогоцінні, за
  замовчуванням «усі»): фільтр, що звужує будь-яке з полів вище одразу, а не окреме
  вимірювання зі своїм списком груп — навмисно не `groupBy`, щоб не тримати дві різні точки
  входу до того самого поняття (рішення власника 2026-09-23, після короткого досвіду з
  `groupBy=metal` як окремим табом). Порожня колекція цілком (`isEmpty`) —
