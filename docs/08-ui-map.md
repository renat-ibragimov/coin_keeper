...
- Везде, где показывается настоящее фото (плитки списков, миниатюры таблиц, карточка
  ...
  ничего не обрезается по краям зоны 16:9. Кадр с реальным фото (не плейсхолдер) стоит на
  светлой фотоплашке (`--color-photo-plaque`, одна и та же в обеих темах — металл лучше
  читается на светлом), а `mix-blend-mode: multiply` на `<img>` растворяет белую студийную
  подложку исходника в плашку без обрезки. Полноценное удаление фона с исходников —
  задача медиапайплайна, отложена.
