...
- **«Каталог і колекція»**: вигляд за замовчуванням окремо для каталогу і для «Мої монети»
  ...
  що в `03-api-contract.md`), і місце зберігання за замовчуванням — `Combobox` зі списком уже
...
переносились: бэкапи — інфраструктурна задача (`10-infra.md`), решта — відкладений
функціонал (`01-scope-mvp.md`, імпорт uCoin і ручне оновлення цін — «Після MVP»).
...
`13-admin.md`. Три вкладки (`?section=jobs|users|proposals`, за замовчуванням
...
на бекенді (`03-api-contract.md`). Немає порожнього/помилкового спеціального стану поза
...
(`03-api-contract.md`). Порожній стан — «Пропозицій ще немає» / «Тут з'являться пропозиції
