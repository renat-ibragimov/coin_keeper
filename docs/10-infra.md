...
разводится на два окружения, и разнесение — часть MVP (`01-scope-mvp.md`,
...
прогоне (`13-admin.md`). Его читает не только бэкенд: контейнер `coin-parser` берёт тот же
...
Три переменные `TELEGRAM_*` — админский бот (`13-admin.md`). Пустой `TELEGRAM_BOT_TOKEN`
...
□ Репозиторий вычищен от русского языка (код и всё вокруг него — CLAUDE.md)
□ Переезд в bakost-numismatics: новое имя репо, coinkeeper → новый технический
  идентификатор (пакеты, контейнеры, образ, compose, секреты CI)
□ Решение о приватности/публичности нового репозитория принято
□ dev (coins.renat-ibragimov.com) и prod (numismatics.bakost.club) разведены,
  БД и медиа продублированы на оба окружения
□ Первый промоушен (украинский каталог + коллекция владельца) в prod выполнен
```
