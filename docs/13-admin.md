...
- **Чем запускать сторожа — решено.** ARQ переехал после MVP целиком, по необходимости
  (решение владельца 2026-09-15, `docs/HANDOFF.md`, §3) — вопрос «поднять ARQ раньше»
  больше не открыт. Сторож — вторым кроном, дёргающим команду в контейнере API, без
  пометки «времянка»: на ARQ он не переедет, пока ARQ не появится в MVP.
