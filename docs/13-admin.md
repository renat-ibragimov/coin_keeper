...
- [ ] Модуль админского бота: транспорт `sendMessage`, реестр получателей, формат
      сообщений
- [ ] Привязка телеграма кнопкой (2.5): вид токена `telegram_link` в `auth_tokens`,
      таблица получателей, выдача ссылки, вебхук с `secret_token`, отвязка
- [ ] Команда `/last` — последний прогон в ответ; маршрутизация под неё всё равно
      пишется ради `/start`
- [ ] Защита от посторонних (2.3, 2.5): отвечаем только своим получателям, всё
      остальное молча игнорируем; настройки в BotFather; токен и секрет вебхука
      в `SERVER_ENV` и в секрете GitHub
