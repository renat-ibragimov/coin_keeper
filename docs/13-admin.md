...
### 2.7. Сторож — отдельная следующая задача

В этой поставке сторожа нет: отсутствие запуска автоматически не вызывает тревогу.
Зависший запуск подсвечивается в интерфейсе через шесть часов. По актуальному решению
2026-09-15 сторож будет отдельным кроном, вызывающим команду API; ARQ для него не нужен.
