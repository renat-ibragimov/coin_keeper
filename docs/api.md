...
- `defaultStorageLocation` (по умолчанию `null`) — имя, не id: сервер резолвит его через
  тот же get-or-create, что и `storageLocation` покупки (`business-rules.md`, п. 16).
...
- `includeSupportingExpenses` (по умолчанию `true`, 2026-09-22) — считать ли сопутствующие
  ...
  и что берёт за базу для расчёта изменения стоимости (`business-rules.md`, п. 4).
...
(`business-rules.md`, пп. 13 и 13а). Причина: выпадашка стран в форме предлагает всех
...
(`NOT is_archived` в запросе, `business-rules.md`, п. 10). При `archived=true`:
...
`collection_item_id` у старых расходов (см. бэкфилл, `business-rules.md`, п. 4). В
...
история цен при архивации **не трогаются** — семантика в `business-rules.md`, п. 10.
...
экземпляром — `business-rules.md`, п. 10.
...
- **материал обязателен** в одном из двух видов: `compositionId` из словаря `GET /materials`
  **или** свободный текст `material`. Ни того ни другого — `422`. Словарь заполнен тем, что
  ...
  `qualityTypeId`). Подробности — `business-rules.md`, п. 14.
...
(`business-rules.md`, п. 4). Курс, валюта и место хранения разрешаются **до** первой
...
покупку целиком, а не за экземпляр (`business-rules.md`, п. 4). Подробности удаления —
там же, п. 10.
...
журнале «Гроші», её удаляет удаление экземпляра (`business-rules.md`, п. 4). Доставка
...
прозрачно (`business-rules.md`, п. 16). `custom` — `true`, если запись принадлежит
...
включая экземпляры архивных позиций (`business-rules.md`, пп. 5 и 10) — то же правило, что
...
нет (`business-rules.md`, п. 7). Эндпоинты ниже работают **только по личным позициям**.
...
к общей. Правила дедупликации — `business-rules.md`, п. 3.
