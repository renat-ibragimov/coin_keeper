...
- `coin_keeper` — ветка `feat/admin-job-visibility`, подготовлена к текущему main. В `main` её нет
...
В выкладке две миграции (`0021` — `job_runs`, `0022` — `telegram_recipients` и новый вид
...
Сверить наличие четырёх новых ключей и сохранность прежних настроек.
Число строк ниже — только вспомогательная проверка, не проверка значений:
...
Работа лежит в ветке, поэтому выкладка — это слияние. Перед слиянием открыть PR в main и дождаться зелёных проверок. CI запускается
на PR; выкладка происходит только после push в main.

```bash
cd ~/Desktop/Work/coin_keeper
git switch main
git pull                                   # на случай, если main ушёл вперёд
git merge --no-ff origin/feat/admin-job-visibility
...
Сторожа «прогон не пришёл» (следующая задача, отдельный cron), редактора каталога, экрана пользователей и
ревью новых монет — это части 2–4, `docs/13-admin.md`.

## Дополнение 2026-09-15

- Новые миграции идут после `0020`: `0021` и `0022`. Применять SQL руками
  или менять существующие номера ревизий на сервере не нужно.
- Если `deploy/.env` парсера содержит настройки UID/сети, передавать его вторым
  `--env-file deploy/.env` при сборке, как делает скрипт ночного запуска.
- Проверить бота также с постороннего Telegram-аккаунта: `/start`, `/last` и
  произвольный текст должны оставаться без ответа. Ссылку подключения не пересылать.
- При снятии роли admin, блокировке аккаунта или отключении Telegram бот больше
  не выдаёт этому чату отчёты.
