...
и общего каталога не касается (`business-rules.md`, п. 2).
...
его запустил (`business-rules.md`, п. 3). Причины две:
...
(`business-rules.md`, п. 10). Экземпляры, покупки и история цен владельцев при этом
...
`created_by = NULL` и видны всем (`business-rules.md`, п. 7).
