...
import { AlertTriangle, Coins, ListChecks, Users } from 'lucide-react';
...
const ALL_JOBS = '';
type AdminSection = 'jobs' | 'users' | 'proposals';
...
export function AdminPage() {
  ...
  const section: AdminSection =
    rawSection === 'users' || rawSection === 'proposals' ? rawSection : 'jobs';
  ...
  function setSection(value: AdminSection) {
    ...
    if (value !== 'jobs') next.set('section', value);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.tabs}>
        <Tabs
          aria-label={t('admin.sections.label')}
          value={section}
          onChange={setSection}
          options={[
            {
      ...
      {section === 'jobs' ? <TelegramCard /> : null}
      {section === 'users' ? (
        <UsersSection page={page} onPageChange={(next) => setParam('page', String(next))} />
      ) : null}
