...
function initialFields(card: CatalogCard): CoinFields {
  return {
    ...
    material: card.composition?.name ?? card.material ?? '',
    metalKind: card.metalKind,
    mintageAnnounced: text(card.mintageAnnounced),
    weightGrams: text(card.weightGrams),
    diameterMm: text(card.diameterMm),
    thicknessMm: text(card.thicknessMm),
    edgeTypeId: text(card.edgeType?.id),
    qualityTypeId: text(card.qualityType?.id),
    shape: card.shape ?? '',
...
export function ProposalEditor({
  ...
  const remove = (side: CoinSide) =>
    setPhotos((current) => ({ ...current, [side]: { kind: 'remove' } }));
  const match = <T extends { id: number }>(
    rows: T[] | undefined,
    value: string,
    label: (row: T) => string,
  ) => {
    const found = rows?.find(
      (row) => label(row).toLocaleLowerCase() === value.trim().toLocaleLowerCase(),
    );
    ...
    onApprove(
      {
        ...
        collectionGroup: values.collectionGroup,
        metalKind: values.metalKind,
        denominationId: denomination.id,
        denominationText: denomination.own,
        seriesId: seriesValue.id,
        seriesText: seriesValue.own,
        compositionId: material.id,
        material: material.own,
        mintageAnnounced: numberOrNull(values.mintageAnnounced),
        weightGrams: nullable(values.weightGrams),
        diameterMm: nullable(values.diameterMm),
        thicknessMm: nullable(values.thicknessMm),
        edgeTypeId: numberOrNull(values.edgeTypeId),
        qualityTypeId: numberOrNull(values.qualityTypeId),
        shape: nullable(values.shape),
  ...
  return (
    <>
      <input
        ref={picker.fileInput}
        type="file"
        accept="image/jpeg,image/png,image/webp"
        hidden
        onChange={picker.onFileChange}
      />
      <Modal
        open={open}
        onClose={onCancel}
        title={t('admin.proposals.editTitle')}
        footer={
          <>
            <Button variant="secondary" onClick={onCancel} disabled={busy}>
              {t('common.cancel')}
            </Button>
            <Button onClick={submit} loading={busy} disabled={!title.trim() || !values.issueYear}>
              {t('admin.proposals.approveChanges')}
            </Button>
          </>
        }
      >
